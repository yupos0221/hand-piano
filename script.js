// ── 音階定義 ───────────────────────────────────────────
const NOTE_MAP = [
  { name: 'ド',   freq: 261.63, fingers: [0, 1], label: '親指＋人差し指', fk: 'f-01', hand: 'Right' },
  { name: 'レ',   freq: 293.66, fingers: [0, 2], label: '親指＋中指',     fk: 'f-02', hand: 'Right' },
  { name: 'ミ',   freq: 329.63, fingers: [0, 3], label: '親指＋薬指',     fk: 'f-03', hand: 'Right' },
  { name: 'ファ', freq: 349.23, fingers: [0, 4], label: '親指＋小指',     fk: 'f-04', hand: 'Right' },
  { name: 'ソ',   freq: 392.00, fingers: [0, 1], label: '親指＋人差し指', fk: 'f-01', hand: 'Left'  },
  { name: 'ラ',   freq: 440.00, fingers: [0, 2], label: '親指＋中指',     fk: 'f-02', hand: 'Left'  },
  { name: 'シ',   freq: 493.88, fingers: [0, 3], label: '親指＋薬指',     fk: 'f-03', hand: 'Left'  },
  { name: 'ド′',  freq: 523.25, fingers: [0, 4], label: '親指＋小指',     fk: 'f-04', hand: 'Left'  },
];
...
const TRANSLATIONS = {
  ja: {
    'reset-btn':'🔄 リセット', 'lang-btn':'EN',
    'placeholder-p':'カメラを起動して<br>指のピンチで演奏しよう',
    'start-btn':'カメラ起動', 'loading-btn':'読み込み中...',
    'cam-toggle-text':'カメラ画像',
    'status-init':'カメラ未起動', 'status-active':'カメラ稼働中', 'status-error':'エラー',
    'cam-error':'カメラにアクセスできませんでした。',
    'oct-panel-lbl':'OCTAVE MODE',
    'oct-btn-off':'オクターブモード OFF', 'oct-btn-on':'オクターブモード ON',
    'oct-ctrl-close':'低 ↔ 中', 'oct-ctrl-far':'中 ↔ 高',
    'oct-size-hint':'手のサイズ（参考）',
    'oct-low':'低', 'oct-mid':'中', 'oct-high':'高',
    'oct-near':'近い', 'oct-normal':'通常', 'oct-far':'遠い',
    'inst-lbl':'INSTRUMENT',
    'inst-piano':'ピアノ', 'inst-trumpet':'トランペット', 'inst-guitar':'ギター',
    'inst-clarinet':'クラリネット', 'inst-flute':'フルート', 'inst-bell':'ベル',
    'ton-lbl':'TONALITY', 'ton-major':'長調', 'ton-minor':'短調',
    'ton-sharp':'♯系', 'ton-flat':'♭系',
    'tilt-lbl':'傾き感度', 'tilt-hint':'左♭ ← → ♯右',
    'guide-lbl':'ピンチ対応表', 'guide-lh':'左手', 'guide-rh':'右手',
    'f-01':'親指＋人差し指', 'f-02':'親指＋中指',
    'f-03':'親指＋薬指',     'f-04':'親指＋小指',
    'hand-r':'右手', 'hand-l':'左手',
  },
  en: {
    'reset-btn':'🔄 Reset', 'lang-btn':'JA',
    'placeholder-p':'Start camera and<br>pinch fingers to play',
    'start-btn':'Start Camera', 'loading-btn':'Loading...',
    'cam-toggle-text':'Camera',
    'status-init':'Camera off', 'status-active':'Camera active', 'status-error':'Error',
    'cam-error':'Could not access camera.',
    'oct-panel-lbl':'OCTAVE MODE',
    'oct-btn-off':'Octave Mode OFF', 'oct-btn-on':'Octave Mode ON',
    'oct-ctrl-close':'Low ↔ Mid', 'oct-ctrl-far':'Mid ↔ High',
    'oct-size-hint':'Hand size (ref)',
    'oct-low':'Low', 'oct-mid':'Mid', 'oct-high':'High',
    'oct-near':'Close', 'oct-normal':'Normal', 'oct-far':'Far',
    'inst-lbl':'INSTRUMENT',
    'inst-piano':'Piano', 'inst-trumpet':'Trumpet', 'inst-guitar':'Guitar',
    'inst-clarinet':'Clarinet', 'inst-flute':'Flute', 'inst-bell':'Bell',
    'ton-lbl':'TONALITY', 'ton-major':'Major', 'ton-minor':'Minor',
    'ton-sharp':'♯ keys', 'ton-flat':'♭ keys',
    'tilt-lbl':'Tilt sens.', 'tilt-hint':'♭ Left ← → Right ♯',
    'guide-lbl':'Pinch Guide', 'guide-lh':'Left Hand', 'guide-rh':'Right Hand',
    'f-01':'Thumb + Index', 'f-02':'Thumb + Middle',
    'f-03':'Thumb + Ring',  'f-04':'Thumb + Pinky',
    'hand-r':'R', 'hand-l':'L',
  },
};
...
function detectingText(n) {
  return currentLang === 'ja'
    ? `手を検出中 (${n}本)`
    : `Detecting (${n} hand${n !== 1 ? 's' : ''})`;
}

function applyTranslations() {
  document.querySelectorAll('[data-i18n]').forEach(el => {
    const key = el.dataset.i18n;
    const sub = el.dataset.i18nSub;
    if (sub) {
      el.innerHTML = `${t(key)}<br><small>${t(sub)}</small>`;
  ...
  if (typeof octaveModeBtn !== 'undefined' && octaveModeBtn) {
    octaveModeBtn.textContent = t(octaveMode ? 'oct-btn-on' : 'oct-btn-off');
  }
  if (typeof statusText !== 'undefined' && typeof video !== 'undefined' && !video?.srcObject) {
    statusText.textContent = t('status-init');
  }
  if (typeof camToggleBtn !== 'undefined' && camToggleBtn && !camToggleBtn.disabled) {
    camToggleBtn.textContent = `📷 ${t('cam-toggle-text')} ${showCameraImage ? 'OFF' : 'ON'}`;
  }
...
let octCloseThr = 0.27;  // refDist > この値 → 近い → 低音域
let octFarThr   = 0.15;  // refDist < この値 → 遠い → 高音域
...
(function buildGuide() {
  ...
  function makeCol(titleKey, notes) {
    ...
    notes.forEach(n => {
      ...
      fSpan.className = 'guide-fingers';
      fSpan.dataset.i18n = n.fk;
      fSpan.textContent = t(n.fk);
      const nSpan = document.createElement('span');
      nSpan.className = 'guide-note';
  ...
  container.appendChild(makeCol('guide-lh', NOTE_MAP.filter(n => n.hand === 'Right')));
  container.appendChild(makeCol('guide-rh', NOTE_MAP.filter(n => n.hand === 'Left')));
...
function onResults(results) {
  ...
  if (results.multiHandLandmarks?.length) {
    ...
    statusText.textContent = detectingText(results.multiHandLandmarks.length);
...
async function startCamera() {
  startBtn.textContent = t('loading-btn');
  ...
  try {
    ...
    camToggleBtn.textContent = `📷 ${t('cam-toggle-text')} OFF`;
    ...
    errorMsg.innerHTML = `<span>⚠️</span><span>${t('cam-error')}<br>${e.message}</span>`;
    statusText.textContent = t('status-error');
    startBtn.textContent = t('start-btn');
